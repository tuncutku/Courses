...
def get_movies(upcoming=False):
    with connection:
        ...
        if upcoming:
            today_timstamp = datetime.datetime.today().timestamp()
            cursor.execute(SELECT_UPCOMING_MOVIES, (today_timstamp,))
        else:
...
def get_watched_movies(username):
    with connection:
        cursor = connection.cursor()
        return cursor.execute(SELECT_WATCHED_MOVIES, (username,)).fetchall()
